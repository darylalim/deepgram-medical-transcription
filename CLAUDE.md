...
- `tests/test_ci_workflow.py` — **static guard** over `.github/workflows/ci.yml` (config-only, no API surface — the workflow is invisible to `ruff`/`ty`/`pytest`, which only look at `.py`): asserts the workflow keeps wiring all four local hook gates (`ruff format --check`/`ruff check`/`ty check`/`pytest`) plus the `uv sync --locked` lockfile-drift check, stays least-privilege (`contents: read`), and interpolates no untrusted `github.event.*`/`github.head_ref` input into a shell step (the GitHub Actions script-injection guard). Text/regex assertions only — **no YAML-parser dependency**.
