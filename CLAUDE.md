...
- **`results.py`** — getattr-guarded response walkers (no `st.*`): `first_alternative`, `transcript_text` (→ `.transcript` or `None`), and `diarized_segments` (groups `alternatives[0].words` into consecutive `(speaker, text)` runs, gated on an integer `words[0].speaker`; `text` uses `punctuated_word` falling back to `word`). **Speaker values from the walkers stay Deepgram's native 0-based ints; the shared display helpers `speaker_label` (the single home of the `+1` offset → `int | None`) and `word_token` (`punctuated_word` → `word`) are reused by the Streamlit renderer and the SRT export.**
