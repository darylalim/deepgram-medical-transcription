...
7. Layout: audio input tabs (Upload ≤100 files/2 GiB each; Record `st.audio_input` ≤10 min; URL HTTP/HTTPS ≤100) full-width above; left **Features** tab (Language, Smart Format, Keyterm Prompting, Diarize, Dictation, Measurements, Redact) + a full-width **Run** button; right **Transcript**/**JSON** tabs via `_output_panel`.
...
- **`main.py`**:
  ...
  - Error envelope `{error: {type, code, message, request_id}}` via handlers for `ApiError`, `RequestValidationError`, `StarletteHTTPException` (routing 404/405 → `not_found`/`method_not_allowed`), and a scrubbed catch-all 500.
